<x-admin-layout title="Edit Category">
    ...
    <div class="space-y-6" x-data="{ 
        ...
        isFeatured: {{ old('is_featured', $category->is_featured) ? 'true' : 'false' }}
    }">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-xl font-black text-slate-900 dark:text-white tracking-tight">Edit Category: <span class="text-[#3c83f6]">{{ $category->name }}</span></h1>
                <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">Update category identity, color branding, display order, and SEO settings.</p>
            </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            @csrf
            @method('PUT')
