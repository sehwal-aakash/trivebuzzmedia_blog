    ...
    <div class="max-w-xl">
        ...
        <div class="bg-white dark:bg-[#0f1729] rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden p-6 md:p-8">
            <div class="mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                <h2 class="text-base font-black text-[#0f1729] dark:text-white uppercase tracking-wider font-sans">Edit Tag</h2>
                <p class="text-xs font-medium text-slate-400 mt-0.5">Editing tag: <span class="text-[#3c83f6] font-bold">#{{ $tag->name }}</span></p>
            </div>
            ...
            <form action="{{ route('admin.tags.update', $tag) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <x-form.input type="text" name="name" id="name" :value="old('name', $tag->name)" required placeholder="e.g. laravel, AI, webdev" />
                    @error('name')
