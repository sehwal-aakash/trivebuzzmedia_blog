<x-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="{ ...aiAssistant(), ...imagePreview('{{ $post->featured_image ? Storage::url($post->featured_image) : '' }}'), ...postEditor() }" x-init="initEditor()" x-effect="onHtmlChange()">
        <form action="{{ route('author.posts.update', $post) }}" method="POST" enctype="multipart/form-data" @submit="onHtmlChange()">
            @csrf
            @method('PUT')

            @if ($errors->any())
                <div class="mb-6 p-4 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-500">
                    <h4 class="font-bold text-sm mb-1">Please fix the following errors before saving:</h4>
